...
**301 映射**:所有 19 个旧 URL → 上表对应新 URL(nginx 层配置;services.html→/solutions/、services/hourly.html 与 cityrides.html→/solutions/business-hospitality/、longterm.html→/solutions/executive-chauffeur/、holding.html→/;其余一一对应)。映射表随实现产出,提交进仓库 `docs/redirects-301.md` 并写进 nginx conf。注:holding.html = 备案窗口期的品牌过渡占位页(历史遗留),**无内容需迁移,仅做 301**。
...
必含 5 个子模块,缺一不可:① 6 车规格表(载客/行李件数/后备箱升数);② 管家培训标准(200+ 学时/30+ 门课程);③ 着装规范(黑色定制西装/白手套/皮鞋);④ 车内物资清单(矿泉水/湿巾/充电线/阅读灯/报刊,每程更换)+ 温度预设/隔断可选;⑤ 环境香氛(S-Class 配备,可选无香)。**车型规格以本页为全站唯一权威数据源**:存于 `src/data/vehicles.json`(与 facts.json/configurator.json 同目录),airport-transfer 等页引用同一来源,禁止各页自维护副本。仓库根 `data/` 目录**仅保留** news.json/careers.json(构建期读取,不搬入 src/data),两套目录职责不混。

### 4.5 LyctAI 页

6 能力(3 Live + 3 Roadmap,量化指标保留:22 字段/2 步映射/7 维度/4 类意图/月报/日简报)→ **活架构图**(§6.3)→ 4 设计原则(AI 不替代承诺/人在决策环上/国产模型优先/可解释可审计)。架构图里"Driver App"文案改为"Chauffeur App / 履约端"。**施工第一步:从现 lyctai.html 提取"6 节点 × (Live/Roadmap 状态 × 量化指标 × hover 文案要点)"映射表**(以现页内容为准,一一对应),入 `src/data/lyctai-capabilities.json`,架构图与能力卡共用。
...
一条单行、等宽字体的事实带,**静态玉色圆点(不做呼吸/脉冲——呼吸灯的视觉语义=运力在线,是网约车读感,禁用)**+ 轮播 3–5 条**历史聚合事实**(如"覆盖 300+ 城市 · 10K+ 企业客户""港深跨境 · 门到门约 90 分钟""上海时装周 · 4 天 80 辆车")。**内容硬规则:禁止出现日期敏感词(今日/现在/实时/当前/在班)与任何运力数量表述**,只允许历史聚合事实与已完成案例。数据源:构建期 JSON(`src/data/facts.json`,**固定收录 5 条,按数组顺序循环播放全部,不随机抽样**);实现层命名一律用 facts/highlights,**禁用 status/live/online 类命名**(防止未来运营顺手填入实时口径)。轮播:8s/条,淡入淡出,hover 暂停。

### 6.2 签名交互 ② 方案顾问(/solutions/,内部代号 configurator)

三步选择:**场景**(7 选 1)→ **城市**(5+其他)→ **人数/行李**(区间)。即时输出:**文字为主的方案说明卡**(建议服务组合 + 建议车型**以图标级小图 + 文字呈现,禁止车型大图卡库式陈列**——那是选车下单页的视觉语言)+ 一句为什么 → CTA 统一用**「获取方案」/「转交商务对接」**(跳联系表单并预填;**禁用"预约/立即用车/叫车"类动词**)。组件顶部常驻双语提示:**"本工具用于生成沟通方案,不构成在线预约或下单。"** 纯前端决策树(`src/data/configurator.json`),无价格、无下单、无实时可用性。选择切换用 250ms 交叉淡入,结果卡从下方 12px 浮入(**组件内转场,独立于 §6.4 滚动 reveal:固定 12px、方向向上、不参与 stagger**)。**过线判定(写进验收)**:请一位非公司人员首次看到该页,5 秒内回答"这是不是能直接叫车"——答案含混即判不合格,重新弱化。
...
4. **摄影**:延续现库(黑色系车辆/管家白手套/城市夜景),新增机场·高铁礼宾题材可后补,先用现库 + 排版补位;**上线满 30 天仍未替换专属题材图,则在 docs/ 留待办并提醒**,不许无限期用错位图。
...
- **i18n**:Astro 内置 i18n 路由(`defaultLocale: 'zh-cn'`, locales zh-cn/en);文案集中在 `src/i18n/` 与内容集合,**禁止**同页双语 span 机制。**双语文案迁移规则**:现站 19 页的 `lang-en`/`lang-cn` span 成对文本,按页拆入 `src/i18n/{locale}/<page>.json`(整句/整段为 key 粒度,key 名以内容语义命名);迁移对照表随实现产出至 `docs/i18n-migration-map.md`,验收按该表抽查双语等价。
...
- **构建与部署**(明确取代旧机制):**本次重建起废弃 deploy-website.sh 的 git-checkout 直 serve 模式**,改为 GitHub Actions 于 push main 时构建并 rsync `dist/` 覆盖 `/var/www/lyctai-website/`,旧脚本降级为仅执行 nginx reload;INFRA.md 与 CLAUDE.md 速查表在切换 commit 中**同步改写**。前置条件(施工范围内):在 ECS 新增部署专用 SSH key(公钥上服务器由用户执行/授权),GitHub 仓库配 secrets。nginx 加 301 映射与 `/en/` 路由。旧站文件在切换 commit 中整体移除(git 历史即备份;备用 tag `pre-rebuild`)。
- **分支纪律**:重建全程在 `feat/rebuild` 分支,**不合 main**;main 只做现网维护(紧急 hotfix、news/careers 内容更新)。验收通过后:先把 main 上开发期间产生的 `data/*.json` 内容更新**同步进重建分支**,再单次 squash-merge 到 main 触发部署;废弃分支保留 7 天后删。
...
8. **连续通读测试**:一名审读者从首页 hero 滚到 footer 一次读完,第一反应必须是"LyctRides 提供出行服务(居间/服务定性)"而非"LyctRides 运营车队(承运定性)";偏向后者则调整模块次序/边界句后复测。**仲裁规则:产品负责人(用户)一票裁决,最多迭代 3 轮;第 3 轮仍不过,输出"必须删除的模块清单"而非继续微调措辞**。
